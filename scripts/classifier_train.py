...
def main():
    ...
    def forward_backward_log(data_loader, step, prefix="train"):
        ...
        for i, (sub_batch, sub_labels, sub_t) in enumerate(
            ...
            total = 5264 + 9509
            weight_healthy = total / (2 * 5264)
            weight_tumor = total / (2 * 9509)

            class_weights = th.tensor([weight_healthy, weight_tumor]).to(sub_labels.device)  # Move to GPU if needed

            loss = F.cross_entropy(logits, sub_labels, reduction="none",weight=class_weights)
            losses = {}
