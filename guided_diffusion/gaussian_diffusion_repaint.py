...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        model_output = model(x = x,ts = self._scale_timesteps(t), y = model_kwargs["y"])
    ...
    def condition_score2(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        ...
        a, cfn= cond_fn(
            x, self._scale_timesteps(t).long(), y = model_kwargs["y"]   
        )
        ...
        if inpainting:
            ...
            for t_last, t_cur in time_pairs:
                ...
                if t_cur < t_last:  # reverse
                    ...
                    # Here, t_last_t corresponds to the “current” timestep, and you need to know the previous timestep (t_prev)
                    # In your schedule, you can use t_cur or compute t_prev accordingly.
                    B = image_after_step.shape[0]
                    t_tensor = th.full((B,), t_last, device=device)    # Create a tensor with B copies of t_last
                    t_prev_tensor = th.full((B,), t_cur, device=device)  # Similarly for t_cur

                    ddim_out = self.ddim_sample(
                        model, image_after_step,
                        t=t_tensor,       # now has shape (B,)
                        t_prev=t_prev_tensor,  # now has shape (B,)
                        clip_denoised=clip_denoised,
                        denoised_fn=denoised_fn,
                        model_kwargs=model_kwargs)

                    # image_after_step = ddim_out["sample"]
    ...
    def ddim_sample_loop_progressive(
        ...
        with th.no_grad():
            if inpainting:
            # Here we use get_schedule_jump from the provided scheduler code.
            # (Adjust the parameters as needed.)
                times= 250
                indices = get_schedule_jump(t_T=times, n_sample=1, jump_length=10, jump_n_sample=10)
                image_after_step = noise if noise is not None else th.randn(*shape, device=device)
                pred_xstart = None
                # get the noisy image until t-1
                indices_2 = list(range(times - 2))[::-1]
                for i in indices_2:
                ...
                # Generate time pairs for repainting
                ...
                for t_last, t_cur in time_pairs:
                    ...
                    if t_cur < t_last:  # Reverse step (denoising)
                        
                            image_before_step = image_after_step.clone()
                            #print(f"self.model type in ddim_sample_with_repaint: {type(model)}")
                            # print('t_last_t in ddim_sample_with_repaint', t_last_t)
                            out = self.ddim_sample_with_repaint(
                                model,
                                image_after_step,
                                t_last_t,
                                clip_denoised=clip_denoised,
                            ...
                            image_after_step = out["sample"]
                            pred_xstart = out["pred_xstart"]
                            yield out  # Yield the intermediate sample

                    else:  # Forward step (adding noise)
                        #print(f"self.model type in ddim_reverse_sample: {type(model)}")
                        image_before_step = image_after_step.clone()
                        t_shift = 1  # Optional time shift for noise reintroduction
                        # print('t_last_t in ddim_reverse_sample', t_last_t)
                        out = self.ddim_reverse_sample(
                            model,
                            image_before_step,
                            t=t_last_t, # removed t_shift
                            clip_denoised=clip_denoised,
                            denoised_fn=denoised_fn,
                            model_kwargs=model_kwargs,
                            eta=eta,
    ...
    def ddim_sample_with_repaint(
        ...
        """
        noise = th.randn_like(x) # X is the image 
        inpa_inj_sched_prev = True

        if inpa_inj_sched_prev:
            # Apply partial replacement with ground truth (gt) if specified
            if pred_xstart is not None:
                gt_keep_mask = model_kwargs.get('gt_keep_mask')
                if gt_keep_mask is None:
                    gt_keep_mask = th.zeros(x)  # Custom mask for ground truth injection
                gt_keep_mask = th.tensor(gt_keep_mask, device=x.device, dtype=x.dtype)
                ...
                # Replace part of x with the weighted ground truth
                x = gt_keep_mask * weighed_gt + (1 - gt_keep_mask) * x

        # Standard DDIM mean and variance prediction
        out = self.p_mean_variance(
            model,
            x,
            t,
