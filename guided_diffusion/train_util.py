...
class TrainLoop:
    ...
    def run_loop(self):
        # step_window = viz.line(
        #     Y=th.zeros(1), 
        #     X=th.zeros(1), 
        #     opts=dict(xlabel='Step', ylabel='Loss', title='Training Loss Over Steps')
        # )

        # batch_window = viz.line(
        #     Y=th.zeros(1), 
        #     X=th.zeros(1), 
        #     opts=dict(xlabel='Batch', ylabel='Loss', title='Training Loss Over Batches')
        # )
        ...
        while (
            ...
            if self.dataset == 'brats':
                try:
                    values = next(self.iterdatal)
                except StopIteration:
                    self.iterdatal = iter(self.datal)
                    values = next(self.iterdatal)
                if len(values) == 3:
                    batch, cond, label = values
                elif len(values) == 5:  # If 5 values are returned
                    batch, cond, label, _, _ = values
                else:
                    raise ValueError(f"Unexpected number of values returned: {len(values)}")
